...
function Support() {
  ...
  const getTickits = async () => {
    try {
      const res = await axios.get(`${config.baseURL}/vendor/settings/support-tickets`, {
        headers: {
          authorization: token,
        },
      })
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.post(
        `${config.baseURL}/vendor/settings/support-tickets`,
        {
      ...
      //   console.error('Error while adding support:', error)
